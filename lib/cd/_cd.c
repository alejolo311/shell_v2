...
int _cd(char **argv, lenv_s **lenv, unsigned int *e)
{
	...
	else if (home != NULL &&  argv[2] != NULL && argv[2][0] == '~')				/* When "cd ~" */
	{
		if (chdir(home) == -1)
		{
			prnerr(argv[0], e, home), free(home), free(pwd), free(prev);
	...
	else if (argv[2] != NULL && argv[2][0] == '-')				/* When "cd -" */
	{
		if (chdir(pwd) == -1)
		{
			if(home != NULL)
				free(home);
			prnerr(argv[0], e, pwd), free(pwd), free(prev);
			return (-1);
		}
		ynprn = 1;
	}
	...
	{   
		if (argv[2] != NULL && argv[2][0] != '-' && argv[2][0] != '~')
		{
			if (chdir(argv[2]) == -1)			/* When "cd path" */
			{
				if(home != NULL)
					free(home);
				prnerr(argv[0], e, argv[2]), free(pwd), free(prev);
				return (-1);
			}	
		}	
	}	
